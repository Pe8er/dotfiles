...
if not token:
    ...
    token_request = requests.post(
        ...
        data='user%5Blogin%5D={u}&user%5Bpassword%5D={p}'.format(
...
if len(streams_xml):  # Is len() here necessary?
    for stream in streams_xml:
        ...
        player_state = str(stream.find('Player').get(
            'state', default='Unknown User'))
        ...
        series_name_long = stream.get(
            'grandparentTitle', default='Unknown Series')
        series_name = ' '.join(series_name_long.split(' ')[
                               :5])  # Only first five words

        # Define how each type of stream is displayed
        stream_formats = dict()
        stream_formats['track'] = '{playerstate} @ {track_artist} @ {track_title} @ {album} @ {duration} @ {position} @ {coverURL} @ {isloved} @ {codec}'
        ...
        display_str = stream_formats[stream.get('type')].format(
            ...
            position=str(stream.get('viewOffset', default='0')),
            isloved=str(stream.get('userRating', default='NA')),
            coverURL=PLEX_SERVER+stream.get(
            ...
            track_artist=stream.get(
                'grandparentTitle', default='Unknown Artist'),
