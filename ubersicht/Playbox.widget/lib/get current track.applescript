			...
			with timeout of 2 seconds
				tell application "Spotify"
					if player state is playing then
						set musicapp to ("Spotify" as string)
						set answer to true
					end if
				end tell
			end timeout
		...
		try
			with timeout of 2 seconds
				tell application "Music"
					if player state is playing then
						set musicapp to ("Music" as string)
						set answer to true
					end if
				end tell
			end timeout
...
on getSongMeta()
	try
		...
		if musicapp is "Spotify" then
			...
			-- Music
		else if musicapp is "Music" then
			set musicAppReference to a reference to application musicapp
			using terms from application "Music"
				try
					tell musicAppReference
			...
			set AppleScript's text item delimiters to "^"
			set {artistName, songName, albumName, currentCoverURL, audioCodec} to {text item 2, text item 3, text item 4, text item 5, text item 6} of plexOutput
			set AppleScript's text item delimiters to ""
...
on grabCover()
	try
		if musicapp is "Music" then
			...
			my getSpotifyArt()
		else if musicapp is "Plexamp" then
